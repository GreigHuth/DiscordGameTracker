...
def update_gametime(user):
	...
	cursor = conn.execute('select * from ' + current_month)
	games = [game[0] for game in cursor.description] # list of all the games in the database

	cursor = conn.execute('select USER_ID from ' +current_month)
